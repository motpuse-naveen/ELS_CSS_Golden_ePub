...
VSlider.prototype.init = function (callbackFn) {
  console.trace()
  if (callbackFn) {
...
VSlider.prototype.handleMouseDown = function (event) {
  ...
  var handleMouseMove = function (event) {
    // console.log(event.pageY, $(self.railDomNode).offset(), self.railDomNode)
    var diffY = 0;
    if (event.type === 'touchmove') {
      diffY = event.touches[0].pageY - $(self.railDomNode).offset().top;
    } else {
      diffY = event.pageY - $(self.railDomNode).offset().top;
    }
...
VSlider.prototype.handleClick = function (event) {
  ...
  var diffY = event.pageY - $(this.railDomNode).offset().top;
